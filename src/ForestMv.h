...
#include "DataMv.h"
 ...
 public:
   ...
   ForestMv(const ForestMv&) = delete;
   ForestMv& operator=(const ForestMv&) = delete;
   ...
   void initR(std::unique_ptr<DataMv> input_data, //uint mtry, 
   ...
   void init(//MemoryMode memory_mode, 
             std::unique_ptr<DataMv> input_data, std::string output_prefix,
             ...
             // bool order_snps, 
             uint max_depth
                // regularization_factor, const std::vector<double>& regularization_factor, bool regularization_usedepth
                );
   void initInternal();
 ...
 protected:
   ...
   void predictTreesInThread(uint thread_idx, const DataMv* prediction_data, bool oob_prediction);
