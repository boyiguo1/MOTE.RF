...
#include "utility.h"
 ...
 TreeMv::TreeMv(std::vector<std::vector<size_t>>& child_nodeIDs, 
 ...
 void TreeMv::init(const Data* data, 
 ...
 void TreeMv::grow(mat* variable_importance) {
     
     // Rcpp::Rcout << "Start TreeMv::grow" << std::endl;        // Debug Line
     ...
     if (!manual_inbag->empty()) {
         ...
         if (sample_with_replacement) {
             // Rcpp::Rcout << "Start TreeMv::bootstrapClassWise" << std::endl;        // Debug Line    
             bootstrapClassWise();
         } else {
             // Rcpp::Rcout << "Start TreeMv::bootstrapWithoutReplacementClassWise" << std::endl;        // Debug Line         
     ...
     // Rcpp::Rcout << "End growing TreeMv" << std::endl;        // Debug Line

 }
 
 
 bool TreeMv::splitNode(size_t nodeID) {
 ...
 bool TreeMv::splitNodeInternal(size_t nodeID) {
 ...
 void TreeMv::bootstrapClassWise() {
 ...
 void TreeMv::bootstrapWithoutReplacementClassWise() {
     // Draw samples for each class
     for (size_t i = 0; i < sample_fraction->size(); ++i) {
         size_t num_samples_class = (*sampleIDs_per_class)[i].size();
         size_t num_samples_inbag_class = round(num_samples * (*sample_fraction)[i]);
 ...
 bool TreeMv::findBestSplit(size_t nodeID) {
 ...
 void TreeMv::findBestSplitValue(size_t nodeID,
 ...
 void TreeMv::predict(const Data* prediction_data, bool oob_prediction) {
