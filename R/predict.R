...
##' @export
predict.MOTE.forest <- function(object, data, predict.all = FALSE,
  ...
  if (!inherits(object, "MOTE.forest")) {
    stop("Error: Invalid class of input object.")
  } else {
    forest <- object
  }
  if (is.null(forest$num.trees) ||
      is.null(forest$child.nodeIDs) || is.null(forest$child.nodes) ||
      # is.null(forest$split.values) || 
      is.null(forest$independent.variable.names) #||
  ...
  if (!is.null(forest$dependent.varID)) {
    ...
    warning("Dependent.varID is not supplied in Forest object")
    # TODO: what convert.pre.xy does?
  ...
  result <- MOTECpp(  x, x_diff, 
                      ...
                      min.node.size, 
                      prediction.mode, forest, replace, 
                      class.weights, 
                      predict.all, keep.inbag, sample.fraction, minprop, holdout, 
                      num.random.splits, oob.error, max.depth, inbag, use.inbag)
...
predict.MOTE <- function(object, data = NULL, predict.all = FALSE,
  ...
  if (is.null(forest)) {
    stop("Error: No saved forest in ranger object. Please set write.forest to TRUE when calling MOTE")
  }
  ## TODO: Delete this
